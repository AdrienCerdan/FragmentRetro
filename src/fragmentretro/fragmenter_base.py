...
class Fragmenter(ABC):
        ...
        for last_comb in last_combinations:
            for node in last_comb:
                for neighbor in self.fragment_graph.neighbors(node):
                    last_comb_copy = set(last_comb).copy()
    ...
    def get_bond_types_for_solution(self, solution: list[CombType]) -> list[tuple[str, str]]:
        """Get the BRICS bond types between fragments in a solution.

        Args:
            solution: A list of fragment combinations forming a complete solution.

        Returns:
            List of (type1, type2) tuples for bonds between solution fragments.
        """
        bond_types = []
        for i, comb1 in enumerate(solution):
            for comb2 in solution[i + 1:]:
                for node1 in comb1:
                    for node2 in comb2:
                        edge_data = self.fragment_graph.get_edge_data(node1, node2)
                        if edge_data:
                            bond_types.append(edge_data["bond_type"])
        return bond_types
